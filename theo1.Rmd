...
#Fonction qui donne le dataset final mais que pour un trajet initial (de A Ã  B)
path <- function(coordA, coordB, grid, dftemp, dfres, t_parcours) {
  ...
  coord <- coordA
  n <- 0
  #dÃ©but de la boucle qui permet de savoir quelles cases ont Ã©tÃ© parcourues par le trajet. On construit ainsi une nouvelle dataset dftemp
  for (i in 1:(niteration+1)) {
    case <- over_2(coord[1],coord[2],grid)
    if (!(case %in% res)) {
      rbind(dftemp, c(case, t_parcours )) # j'ai que trouvÃ© Ã§a pour avoir une dataframe oÃ¹ on aura les cases d'un cotÃ© et les temps de parcours de l'autre pour chaque trajet mais sans faire un truc Ã©norme et vide
    }
...
#fonction qui nous donnera la dataframe dans le format qu'on veut (dfres) Ã  partir de la dataframe initiale(dfinit, c'est train et test quoi)
temps_trajet_case <- function(dfinit,dfres, grid){
  df <- dfinit %>% 
    ...
    mutate(path = path(coordA, coordB, grid, dftemp, dfres, trip_duration)) #Bon je sais pas si c'est de cette forme mais Ã§a buggue - l'idÃ© c'est d'appliquer path Ã  chaque ligne de la dataframe
}
